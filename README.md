...
4. **fleet-config-lite** — clone and run its `install.ps1` so the session-state hook and `/issue-start` / `/issue-yolo` skills exist; confirm `~/.copilot/hooks/state/sessions-state.json` appears after the first Copilot session.
