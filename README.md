...
- **Brand assets** — this fork does not generate its icons; it commits upstream [`app-launcher`](https://github.com/ferraroroberto/app-launcher)'s rendered PWA/tray/Stream-Deck set verbatim, deliberately avoiding a `project-scaffolding` dependency. `.\.venv\Scripts\python.exe scripts\gen_icons.py` verifies the committed copies still match an upstream checkout (sibling `..\app-launcher`, or `--upstream` / `$env:APP_LAUNCHER_DIR`) and exits non-zero on drift; `--sync` copies the changed ones over for review. Not part of the pre-ship gate — run it when upstream's brand changes.
