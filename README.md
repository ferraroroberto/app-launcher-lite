...
## Work-machine handoff checklist

This fork was built and tested on the home machine with the GitLab/Copilot specifics mocked where the real services weren't reachable. On the actual work machine, walk through:

1. **glab against the work host** — `glab auth login` with the corporate GitLab, set `gitlab_host` + `gitlab_group`, then validate one real response shape: run `glab api "groups/<group>/issues?state=opened&per_page=1"` and compare fields against the fixtures in `tests/test_gitlab_client.py` (`references.full`, `closed_at`, `web_url`). If the instance returns a different shape, fix the fixtures and `src/gitlab_client.py` together.
2. **Copilot login + model gate** — log the `copilot` CLI in with the work account, then check which of `gpt-5.6-luna` / `gpt-5.6-terra` / `gpt-5.6-sol` the tenant actually offers; trim `copilot_models` in `webapp_config.json` to the real set (an ungated id fails visibly at launch). If none are gated in, set `copilot_model` to `""` (auto).
3. **Real team-os repo** — point `team_os_dir` at the actual team-os checkout (skills under `.claude/skills`).
4. **fleet-config-lite** — clone and run its `install.ps1` so the session-state hook and `/issue-start` / `/issue-yolo` skills exist; confirm `~/.copilot/hooks/state/sessions-state.json` appears after the first Copilot session. **Known CLI limitation (1.0.70, home-verified):** the hooks fire reliably in `copilot -p` runs but NOT in interactive TUI sessions — upstream bugs [copilot-cli#991](https://github.com/github/copilot-cli/issues/991) / [#2201](https://github.com/github/copilot-cli/issues/2201). Until a CLI update fixes that, Board sessions show status "unknown" (presence still works); re-test with the work machine's CLI version and after each `copilot update`.
5. **Boot autostart** — flip the Settings toggle; verify `webapp/startup.log` gets a breadcrumb on the next login.
6. **Un-skip the real-agent echo probe** — `tests/e2e/test_terminal_reconnect.py`'s reconnect-replay test is `@pytest.mark.skip`-ped pending a machine where a real `copilot` boot works end-to-end; re-enable it there and tune `E2E_REAL_AGENT_ECHO_MS` if needed.
