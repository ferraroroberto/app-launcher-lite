...
**App Launcher Lite** — phone-first launcher hub, a lite fork of [ferraroroberto/app-launcher](https://github.com/ferraroroberto/app-launcher) for Copilot-only environments. Setup and usage: `README.md`; deep references: `docs/board.md`, `docs/jobs-tab.md`.

- **One coding agent only:** GitHub Copilot CLI (`src/agents.py`; config-driven `copilot_models` list, default `gpt-5.6-luna`). **One forge only:** GitLab via `glab` (`src/gitlab_client.py`; `gitlab_group` / `gitlab_host` config).
- **Five work tabs plus Settings** — Coding (live PTY / detached Copilot sessions via the `:8466` session-host, project list from `projects_dir`) · Apps (registry `config/apps.json`: `streamlit` | `webapp` | `tunnel` | `tray`) · Jobs (`config/jobs.json`, Task Scheduler-backed) · Team OS (skills from `<team_os_dir>/.claude/skills`, launched as Copilot sessions) · Board (4 columns Backlog · Bot's turn · Your turn · Done; GitLab issues via `glab` on demand, session status from fleet-config-lite's `sessions-state.json`).
- **Companion repo** [fleet-config-lite](https://github.com/ferraroroberto/fleet-config-lite): Copilot hooks (`~/.copilot/hooks/state/`), the `/issue-start` / `/issue-yolo` skills the Board's ▶/⚡ buttons launch, `install.ps1`.
- **Stack:** FastAPI + vanilla JS — **not** Streamlit; do not introduce Streamlit. Windows-only (ConPTY via pywinpty).
- **Config & secrets:** no `.env`. `config/config.json` (app-level), `config/webapp_config.json` (runtime UI prefs + secrets), `config/apps.json` (app registry), `config/jobs.json` — all gitignored with committed `*.sample.json` templates. The `webapp_config.json` field list lives in `src/webapp_config.py` (dataclass + serializer); keep the sample and README table in sync with it.
...
- **Verification:** before declaring any webapp/launcher/session-host change done, run the pre-ship gate `pwsh -File scripts/verify-before-ship.ps1` (boots its own disposable webapp + session-host — no tray needed). Byte-compile + the non-e2e pytest suite always run; the browser slice is routed by `scripts/classify_e2e.py` against `main` — static-asset-only → Chromium-only smoke; backend/docs-only → no browser suite; any `.js`/`.css`/real-page/session-host/e2e-test change (or any ambiguous diff) → the full suite, Chromium driving both a desktop and a phone (Android device-emulated) projection (Android-only fork, no WebKit/iPhone engine). Routing is fail-safe: uncertainty escalates to full, never narrows; the chosen tier prints to console and `webapp/verify-progress.log`. Dev-loop smoke against the live tray: `.\scripts\run-e2e.ps1` (sets `LAUNCHER_E2E_LIVE=1`; a bare `pytest tests/e2e` without opt-in env vars exits with a guard). Non-browser suite alone: `pytest tests -m "not smoke" -v`.
- **Restart and verify before hand-off:** the webapp has no hot-reload — code edits do nothing until the `:8465` process restarts. Canonical restart: **`tray.bat --restart`** (orphan-proof reclaim-then-start; run it via the PowerShell tool, not Git Bash). It kills the tray subtree and reclaims `:8465` by `.venv`-scoped PID, but **preserves the `:8466` session-host and every live PTY session** — spawned detached, excluded from the reclaim sweep. Then confirm the new build with a bounded poll of `GET /api/version` (`git_sha` matches `HEAD`, `asset_hash` changed) and report that line.
- **A session-host change is not live until `:8466` itself restarts — `tray.bat --restart` never does that.** Read `GET /api/version`'s `session_host.stale_relevant` (defined below): `true` → report the change as **merged but not yet live**, never as shipped; `false` with `stale: true` → the repo moved on but nothing the session-host loads changed. The one supported restart is `pwsh -File scripts/restart-session-host.ps1 -Confirm` — operator-initiated only, at a clean boundary: it kills every live PTY on the machine. Never wire it into a normal ship flow.
...
- accepted exceptions:            # permanent, re-confirmed by each /design-sync run
  - `app-icon-family` FAILs by design and stays FAIL (issues #10, #28 — re-filed once already; **do not re-file this as drift**). Its last sub-check requires a `scripts/*.py` calling `project-scaffolding`'s `brand_gen.render_set`; that module is unavailable in this fork and adding it back is forbidden — avoiding the `project-scaffolding` dependency is *why* the fork exists. Every other sub-check passes: the seven rendered assets (`app/webapp/static/icon-180|192|512|512-maskable.png`, `favicon.ico`, `assets/tray/app-launcher.ico`, `assets/stream-deck/app-launcher-144.png`) are committed byte-for-byte from upstream `app-launcher`; index + manifest wiring is correct. Re-sync when upstream's brand changes with `scripts/gen_icons.py` (verify by default, `--sync` to copy; upstream checkout = sibling `../app-launcher`, or `--upstream` / `APP_LAUNCHER_DIR`) — never by re-adopting the generator, and never by faking a `render_set(` call to turn the contract green.
...
- Convention — mock non-deterministic boot fetches before `goto()`, don't widen timeouts; never assert on a raw `evaluate()`/`bounding_box()` read of a polled-re-render surface (board columns rebuild every 5 s) — prefer auto-retrying `expect()` assertions, or wrap unavoidable raw reads in `tests/e2e/conftest.py::stable_read`.
- CI's only signal beyond the local gate is the e2e suite; its surface = `app/webapp/`, the session-host/PTY layer (`src/session_host*.py`, `src/launcher.py`), `tests/e2e/`, static assets — a diff touching none of these gains nothing from CI.
