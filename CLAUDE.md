...
- **Restart and verify before hand-off:** the webapp has no hot-reload — code edits do nothing until the `:8465` process restarts. Canonical restart: **`tray.bat --restart`** (orphan-proof reclaim-then-start; run it via the PowerShell tool, not Git Bash). It kills the tray subtree and reclaims `:8465` by `.venv`-scoped PID, but **preserves the `:8466` session-host and every live PTY session** — the session-host is spawned detached and excluded from the reclaim sweep. After restarting, confirm the new build with a bounded poll of `GET /api/version` (`git_sha` matches `HEAD`, `asset_hash` changed) and report that line.
