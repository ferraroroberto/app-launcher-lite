...
live session mirrors while the sessions survive headless on the real ``:8466``.
...
def _app(session_host_port: int = 8466) -> SimpleNamespace:
...
async def test_canonical_instance_reconciles_against_live_sids(monkeypatch):
    """Without the override (the real :8465 webapp), the sweep still runs and is
