...
def _copilot_installed(monkeypatch):
    """Launch routes 400 when the agent CLI isn't on PATH — including the
    default agent (the lite fork has no unguarded core agent). Default every
    test in this module to "installed" so launches exercise the spawn path;
    the not-installed tests re-patch to False locally."""
...
class TestGetApps:
    ...
    def test_lists_seeded_entries(self, webapp_client):
        """Bat rows come from the registry; coding rows are scanned
        live from projects_dir — both surface in /api/apps. Coding
        ...
        # "beta" stays "beta" — no prettification for coding rows.
        assert names == {"Alpha", "beta"}


class TestCodingDiscovery:
    """The Coding tab lists projects_dir's child directories live —
    no scan step, no persistence in apps.json (issue #44)."""

    def test_child_dirs_appear_as_coding_rows(self, webapp_client):
        client, _, overrides = webapp_client
        for name in ("proj-one", "proj-two"):
            (overrides["tmp_projects_dir"] / name).mkdir()
        apps = client.get("/api/apps").json()["apps"]
        cc = [a for a in apps if a["kind"] == "coding"]
        assert {a["project_dir"] for a in cc} == {
            str(overrides["tmp_projects_dir"] / "proj-one"),
            str(overrides["tmp_projects_dir"] / "proj-two"),
        }

    def test_stale_coding_rows_in_registry_are_ignored(self, webapp_client):
        """An older apps.json may still carry coding rows — the API
    ...
    def test_projects_ignore_hides_matching_dirs(self, webapp_client):
        ...
        apps = client.get("/api/apps").json()["apps"]
        cc_names = {a["name"] for a in apps if a["kind"] == "coding"}
    ...
    def test_launch_resolves_live_coding_dir(
        self, webapp_client, monkeypatch
    ):
        """A coding row isn't in the registry — launch must resolve
        ...
        def fake_spawn(project_dir, name, flags, port, kind="pty", agent="copilot"):
            captured["project_dir"] = str(project_dir)
            captured["kind"] = kind
            captured["agent"] = agent
            return {"session_id": "s1", "kind": kind, "agent": agent}

        monkeypatch.setattr(apps_router, "spawn_agent_session", fake_spawn)
        ...
        # No `agent` in the body → defaults to Copilot.
    ...
    def test_launch_pty_forwards_phone_dimensions(
        ...
        spawn_agent_session so the PTY spawns at the right size (issue
        ...
        def fake_spawn(
            project_dir, name, flags, port, kind="pty", agent="copilot",
    ...
    def test_launch_with_copilot_agent(self, webapp_client, monkeypatch):
        ...
        spawn_agent_session, and its --allow-all toggle composes in."""
        client, _, overrides = webapp_client
        from app.webapp.routers import apps as apps_router
...
class TestLaunchSpawnErrorMapping:
    ...
    def test_session_host_error_status_is_passed_through(
        ...
        monkeypatch.setattr(apps_router, "spawn_agent_session", boom)
...
class TestLaunchAppTracksSpawn:
    """Non-coding launches must register the spawn with app_runtime
