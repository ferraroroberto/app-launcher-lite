...
2. Restart the `:8465` webapp. The fleet hash is computed **once at startup** — not per request — so an unrestarted process keeps serving the old hash even though the files on disk changed.
